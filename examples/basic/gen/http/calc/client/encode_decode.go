...
// set to call the "calc" service "concat" endpoint
func (c *Client) BuildConcatRequest(ctx context.Context, v interface{}) (*http.Request, error) {
	...
	{
		p, ok := v.(*calcsvc.ConcatPayload)
		if !ok {
			return nil, goahttp.ErrInvalidType("calc", "concat", "*calcsvc.ConcatPayload", v)
		}
		a = p.A
		b = p.B
	}
	u := &url.URL{Scheme: c.scheme, Host: c.host, Path: ConcatCalcPath(a, b)}
	req, err := http.NewRequest("GET", u.String(), nil)
	if err != nil {
		return nil, goahttp.ErrInvalidURL("calc", "concat", u.String(), err)
	}
	if ctx != nil {
		req = req.WithContext(ctx)
	}
...
// DecodeConcatResponse returns a decoder for responses returned by the calc
...
func DecodeConcatResponse(decoder func(*http.Response) goahttp.Decoder, restoreBody bool) func(*http.Response) (interface{}, error) {
	return func(resp *http.Response) (interface{}, error) {
		...
		case http.StatusOK:
			...
			err = decoder(resp).Decode(&body)
			if err != nil {
				return nil, goahttp.ErrDecodingError("calc", "concat", err)
			}
			return body, nil
		default:
			body, _ := ioutil.ReadAll(resp.Body)
			return nil, goahttp.ErrInvalidResponse("calc", "concat", resp.StatusCode, string(body))
		}
	}
}
